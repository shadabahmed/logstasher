...
describe LogStasher::ActiveSupport::LogSubscriber do
  ...
  describe '.process_action' do
    ...
    let(:logger)  { double }
    let(:json)    do
      "{\"request\":true,\"status\":true,\"runtimes\":true,\"location\":true,\"exception\":true,\"custom\":true,\"source\":\"unknown\",\"tags\":[\"request\"],\"@timestamp\":\"#{$test_timestamp}\",\"@version\":\"1\"}\n"
    end
    before do
  ...
  describe 'logstasher output' do
    ...
    it 'should add a valid status when an exception occurred' do
      raise AbstractController::ActionNotFound, 'Could not find an action'
    # working this in rescue to get access to $! variable
      ...
      expect(log_output.json['error']).to be =~ %r{AbstractController::ActionNotFound.*Route not found.*logstasher.*/spec/lib/logstasher/active_support/log_subscriber_spec\.rb}m
      expect(log_output.json['tags']).to include 'request'
